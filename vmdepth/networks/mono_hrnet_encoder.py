...
from vmdepth.layers import BackprojectDepth, Project3D
import logging
...
def _hrnet(arch, pretrained, progress, **kwargs):
    ...
    if pretrained:
        if arch == 'hrnet64':
            ...
            # 因执行官方写法报错，所以按照下述来写
            model_ = timm.create_model('hrnet_w18', pretrained=False) 
            # 当前脚本路径
            current_dir = os.path.dirname(os.path.abspath(__file__))

            # 回退两级目录
            root_dir = os.path.abspath(os.path.join(current_dir, "../../pretrained"))

            # 拼接权重路径
            weight_path = os.path.join(root_dir, "hrnetv2_w18-8cb57bb9.pth") 
            print("weight_path:",weight_path)
            state_dict = torch.load(weight_path, map_location="cpu") 
            # pdb.set_trace()
            model_.load_state_dict(state_dict) 
            loaded_state_dict = model_.state_dict()
