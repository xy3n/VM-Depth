...
from .layers import SSIM, BackprojectDepth, Project3D, transformation_from_parameters, \
    compute_rigid_flow, disp_to_depth, get_smooth_loss, flow_warp, image_similarity, L2_norm, flow_smooth_loss

from vmdepth import datasets, networks
import matplotlib.pyplot as plt
                           
from vmdepth import rigid_warp as tool
from vmdepth.evaluate_depth import evaluate
...
class Trainer:
    def __init__(self, options):
        ...
        self.is_multi = None                                    
        self.models["encoder"] = networks.ResnetEncoderMatching_FusedCostVolume_VMamba(
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.num_epochs):
            ...
            if (self.epoch + 1) % self.opt.save_frequency == 0:
                ...
                if (self.epoch + 1) > 0:
                    self.test_epoch_student_outside_every_epoch(save_folder)                       
    ...
    def test_epoch_student_outside_every_epoch(self, save_folder):
    ...
    def compute_losses(self, inputs, outputs, is_multi=False):
        ...
        for scale in self.opt.scales:
            ...
            if self.is_multi and (self.epoch + 1)  > self.opt.freeze_teacher_epoch:
                                                                                                                      
                depth = outputs[("depth",0, scale)] 
    
                pseudo_depth = inputs['pseudo_depth'].unsqueeze(1) 
                                  
                pseudo_depth = F.interpolate(
                    pseudo_depth, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
                pred_depth = F.interpolate(
                    depth, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
                loss_distill =  0.1*self.ssi_loss(pred_depth, pseudo_depth)
                losses['distill_loss/{}'.format(scale)] =loss_distill
                loss += loss_distill
